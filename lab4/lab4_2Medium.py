def limit_calls(max_calls=None):  # 1. ИЗМЕНЕНИЕ: параметр стал опциональным
    ...
    def decorator(func): #объявляем функцию decorator oна принимает параметр func- функция, которую мы декорируем
        ...
        in_recursion = False  # 2.ИЗМЕНЕНИЕ: флаг для отслеживания рекурсии который показывает находимся внутри рекурсивного вызова или нет
        def wrapper(*args, **kwargs):
            ...
            if in_recursion: # 4.ИЗМЕНЕНИЕ: проверяем, не рекурсивный ли это вызов
                return func(*args, **kwargs)#5.ИЗМЕНЕНИЕ: если лимит не задан - просто вызываем функцию
            if calls >= max_calls:
